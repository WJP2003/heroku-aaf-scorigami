		...
		if(req.url != "/favicon.ico") {
			...
			res.write(data,function() {
				fs.readFile("scores.txt",function(err2,data2) {
					if(!err2) {
						res.write(data2 + "]; for(var i = 0;i < arr.length;i++) { document.getElementsByClassName('col' + Math.max(arr[i][0],arr[i][1]) + ' row' + Math.min(arr[i][0],arr[i][1]))[0].classList.remove('never'); }  </script></html>",function() {
							res.end();
						});
					} else {
